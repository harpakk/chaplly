    ...
    <main>
      ...
      <section className="shops-section" id="shops">
        <div className="shop-container">
          ...
          <div className="shop-grid">{shops.map((shop) => {const own=products.filter((item)=>item.shopSlug===shop.slug);const preview=(own.length?own:products).slice(0,3);return <article className="shop-card" key={shop.id}><Link className="shop-cover" href={`/stores/${shop.slug}`}><Image src={shop.banner} alt={`بنر ${shop.name}`} fill sizes="400px" /></Link><div><span className="shop-avatar"><Image src={shop.logo} alt={`لوگوی ${shop.name}`} fill sizes="52px" /></span><small>{shop.handle}</small><h3>{shop.name}</h3><p>{shop.bio}</p><div className="shop-product-peek">{preview.map((product)=><Link href={`/products/${product.slug}`} key={product.id}><Image src={product.image} alt={product.title} fill sizes="90px"/><span>{product.title}</span></Link>)}</div><Link href={`/stores/${shop.slug}`}>دیدن فروشگاه <ArrowLeft /></Link></div></article>})}</div>
        </div>
      </section>
