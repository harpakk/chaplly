-- Make final product creation safe to retry after a later metadata/image step
-- fails, and return stable domain errors instead of leaking constraint names.
...
declare
  ...
  v_requested_product uuid:=nullif(p_payload->>'productId','')::uuid;
  v_product uuid;
  ...
  v_discounted_price bigint:=nullif(p_payload->>'discountedPrice','')::bigint;
...
begin
  ...
  -- A retry without an explicit id resumes the latest unfinished product for
  -- this design. The application normally sends the id, but this also makes
  -- direct RPC retries idempotent.
  if v_requested_product is null then
    select id into v_product
    from public.seller_products
    where store_id=v_store and design_id=v_design and raw_product_id=v_raw
      and status in ('DRAFT','PENDING')
    order by updated_at desc
    limit 1
    for update;
    v_product:=coalesce(v_product,gen_random_uuid());
  else
    v_product:=v_requested_product;
  end if;

  if exists(select 1 from public.seller_products where id=v_product)
    and not exists(
      select 1 from public.seller_products
      where id=v_product and store_id=v_store
        and design_id=v_design and raw_product_id=v_raw
    ) then
  ...
  if length(trim(coalesce(p_payload->>'description','')))=0 then
    raise exception 'DESCRIPTION_REQUIRED';
  end if;
  if v_price<=0 then
    raise exception 'VALID_PRICE_REQUIRED';
  end if;
  if v_discounted_price is not null
    and (v_discounted_price<0 or v_discounted_price>=v_price) then
  ...
  v_slug:=trim(both '-' from lower(
    regexp_replace(
      coalesce(nullif(trim(p_payload->>'slug'),''),'product-'||substr(v_product::text,1,8)),
      '[^a-z0-9-]+','-','g'
    )
  ));
  if length(v_slug)<3 or length(v_slug)>80
    or v_slug !~ '^[a-z0-9][a-z0-9-]*[a-z0-9]$' then
    raise exception 'SLUG_INVALID';
  end if;
  if exists(
    select 1 from public.seller_products
    where store_id=v_store and slug=v_slug and id<>v_product
  ) then
  ...
  insert into public.seller_products(
    ...
    nullif(trim(p_payload->>'seoDescription'),'')
  )
  ...
  delete from public.seller_product_variants
  where seller_product_id=v_product
    and not exists(
      select 1 from public.design_variants
      where design_id=v_design
        and raw_product_variant_id=seller_product_variants.raw_product_variant_id
    );
