create or replace function public.service_upsert_raw_product_mockup(
  ...
  p_color_id uuid,p_gender text,p_perspective_points jsonb default null
)
...
begin
  ...
  insert into public.raw_product_mockup_views(mockup_id,side,background_file_id,area_x,area_y,area_width,area_height,rotation_degrees,perspective_points) values(p_id,p_side,background_id,p_area_x,p_area_y,p_area_width,area_height,p_rotation_degrees,p_perspective_points) on conflict(mockup_id) do update set side=excluded.side,background_file_id=excluded.background_file_id,area_x=excluded.area_x,area_y=excluded.area_y,area_width=excluded.area_width,area_height=excluded.area_height,rotation_degrees=excluded.rotation_degrees,perspective_points=excluded.perspective_points,updated_at=now();
  return p_id;
end $$;
revoke all on function public.service_upsert_raw_product_mockup(uuid,uuid,text,text,uuid,numeric,numeric,numeric,numeric,uuid,uuid,text,jsonb) from public,anon,authenticated;
grant execute on function public.service_upsert_raw_product_mockup(uuid,uuid,text,text,uuid,numeric,numeric,numeric,numeric,uuid,uuid,text,jsonb) to service_role;
