...
as $$
  with bounds as (
    ...
    select distinct store.organization_id
    ...
    where earning.earning_type = 'SELLER' and earning.status <> 'REVERSED'
    group by earning.beneficiary_organization_id
  ), seller_metrics as (
    select organization.id, organization.display_name as name,
      (product_creators.organization_id is not null) as has_product,
      coalesce(sales.items, 0)::bigint as items,
      coalesce(profits.lifetime_profit, 0)::bigint as lifetime_profit
    from public.organizations organization
    left join product_creators on product_creators.organization_id = organization.id
  ...
  select jsonb_build_object(
    ...
    'lifetimeUniqueVisitors', (
      select count(distinct visit.visitor_hash)
      from public.storefront_unique_visits visit
    ),
    'allSellers', (select seller_count from totals),
    'createdProduct', count(*) filter (where has_product),
    ...
    'averageLifetimeProfit', (
      select case when seller_count > 0 then round(total_profit::numeric / seller_count) else 0 end from totals
    ),
    'sellerRows', coalesce(jsonb_agg(jsonb_build_object(
      'organizationId', id, 'name', name, 'itemsSold', items,
